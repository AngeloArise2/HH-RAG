...
- [x] **Deployed latency meets spec**: retrieval-only P50/P70/P100 = **14.8 / 16.9 / 27.1ms** measured against the live URL (20 real queries, `scripts/bench_remote.py`) — inside the 200ms budget. First benchmark read 824ms p50 and was misattributed to hardware; root-caused to two real defects (per-request Chroma collection reopen; ORT threads auto-sized to host cores on a 0.1-CPU container), fixed in `2f66af8`. Full before/after: `docs/latency_report.md` "Deployed latency"
