...
- [x] **Render redeployed and now the PERMANENT home** (Aug 22, 2026): https://voice-rag-j9oh.onrender.com via `render.yaml` blueprint; secrets in Render's vault (`sync:false`, never in git). Why it works now when it didn't before: the memory fix — 283MB < 512MB with ~45% headroom
- [x] **Deployed latency meets spec**: retrieval-only P50/P70/P100 = **10.0 / 12.0 / 17.3ms** measured against the live URL (20 real queries, all three retrieval stages incl. chunk_assembly, `scripts/bench_remote.py`) — inside the 200ms budget. First benchmark read 824ms p50 and was misattributed to hardware; root-caused to three real defects (per-request Chroma collection reopen; ORT threads auto-sized to host cores on a 0.1-CPU container; a chunk-assembly stage documented but never wired into timing), fixed in `2f66af8` + `af463e4`. Full arc: `docs/latency_report.md` "Deployed latency"
